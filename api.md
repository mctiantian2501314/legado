# 阅读[API](/app/src/main/java/com/eggreader/app/api/controller)

## 对于[Web](/app/src/main/java/com/eggreader/app/web/)的配置
...
请求BODY内容为`JSON`字符串，  
格式参考[这个文件](/app/src/main/java/com/eggreader/app/data/entities/BookSource.kt)
...
请求BODY内容为`JSON`字符串，  
格式参考[这个文件](/app/src/main/java/com/eggreader/app/data/entities/BookSource.kt)，**为数组格式**。
...
请求BODY内容为`JSON`字符串，  
替换规则参考[这个文件](/app/src/main/java/com/eggreader/app/data/entities/ReplaceRule.kt)。
...
请求BODY内容为`JSON`字符串，  
格式参考[这个文件](/app/src/main/java/com/eggreader/app/data/entities/Book.kt)。
...
请求BODY内容为`JSON`字符串，  
格式参考[这个文件](/app/src/main/java/com/eggreader/app/data/entities/BookProgress.kt)。
...
### [Content Provider](/app/src/main/java/com/eggreader/app/api/ReaderProvider.kt)


* 需声明`io.legado.READ_WRITE`权限
* `providerHost`为`包名.readerProvider`, 如`com.eggreader.app.release.readerProvider`,不同包的地址不同,防止冲突安装失败
